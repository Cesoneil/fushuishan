    ...
    <?php if ($level >= 1) { ?>
        <div class="col-lg-<?= $col; ?>">
            <?= $form->field($model, $provincesName)->dropDownList(Yii::$app->services->provinces->getCityMapByPid(), [
                'prompt' => '-- 请选择省 --',
                'onchange' => "widget_provinces(this, 1, '$cityId', '$areaId', '$townshipId', '$villageId','$level')",
            ]); ?>
        </div>
    <?php } ?>
    <?php if ($level >= 2) { ?>
        <div class="col-lg-<?= $col; ?>">
            <?= $form->field($model, $cityName)->dropDownList(Yii::$app->services->provinces->getCityMapByPid($model->$provincesName, 2), [
                'prompt' => '-- 请选择市 --',
                'onchange' => "widget_provinces(this, 2, '$cityId', '$areaId', '$townshipId', '$villageId','$level')",
            ]); ?>
        </div>
    <?php } ?>
    <?php if ($level >= 3) { ?>
        <div class="col-lg-<?= $col; ?>">
            <?= $form->field($model, $areaName)->dropDownList(Yii::$app->services->provinces->getCityMapByPid($model->$cityName, 3), [
                'prompt' => '-- 请选择区 --',
                'onchange' => "widget_provinces(this, 3, '$cityId', '$areaId', '$townshipId', '$villageId','$level')",
            ]) ?>
        </div>
    <?php } ?>
    <?php if ($level >= 4) { ?>
        <div class="col-lg-<?= $col; ?>">
            <?= $form->field($model, $townshipName)->dropDownList(Yii::$app->services->provinces->getCityMapByPid($model->$areaName, 4), [
                'prompt' => '-- 请选择乡/镇 --',
                'onchange' => "widget_provinces(this, 4, '$cityId', '$areaId', '$townshipId', '$villageId','$level')",
            ]) ?>
        </div>
    <?php } ?>
    <?php if ($level >= 5) { ?>
        <div class="col-lg-<?= $col; ?>">
            <?= $form->field($model, $villageName)->dropDownList(Yii::$app->services->provinces->getCityMapByPid($model->$townshipName, 5), [
                'prompt' => '-- 请选择村/社区 --',
                'onchange' => "widget_provinces(this, 5, '$cityId', '$areaId', '$townshipId', '$villageId','$level')",
            ]) ?>
        </div>
    <?php } ?>
</div>
...
<script>
    function widget_provinces(obj, type_id, cityId, areaId, townshipId, villageId, level) {
        switch (level) {
            case 1 :
        ...
        var pid = $(obj).val();
        traversal_address(type_id, pid, level);    //修复地址选择出现的bug
        function traversal_address(type_id, pid, level) {
            if (type_id < level) {
               // console.log(type_id + '--7--' + level);
                $.ajax({
                    type: "get",
                    url: "<?= $url; ?>",
                    dataType: "json",
                    ...
                    success: function (data) {
                        ...
                        if (type_id < level) {
                            type_id++;
                           //console.log(type_id + '--6--' + level);
                            traversal_address(type_id, '',level);
                        }
                    }
                });
            }
